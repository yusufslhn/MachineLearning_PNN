...
data_train1 = xlsread('data train.xlsx','Sheet1','A1:D150');
data_test = xlsread('data test.xlsx','Sheet1','A1:C30');

class = xlsread('data train.xlsx','Sheet1','D1:D100');
data_train = xlsread('data train.xlsx','Sheet1','A1:D100');
datatrainjadiuji(:,1:3) = data_train1(101:150,1:3);
...
% -----------------------------
% % menghitung probabilitas nya 
% -----------------------------
x = 1;
for i = 1:size(datatrainjadiuji,1)
    tabelhasil(x,1) = fungsiPNNbuattesting(taw0,size(datatrainjadiuji,1),datatrainjadiuji(i,1),datatrainjadiuji(i,2),datatrainjadiuji(i,3),kelas0,kelas0,kelas0,size(kelas0,1));
    tabelhasil(x,2) = fungsiPNNbuattesting(taw1,size(datatrainjadiuji,1),datatrainjadiuji(i,1),datatrainjadiuji(i,2),datatrainjadiuji(i,3),kelas1,kelas1,kelas1,size(kelas1,1));
    tabelhasil(x,3) = fungsiPNNbuattesting(taw2,size(datatrainjadiuji,1),datatrainjadiuji(i,1),datatrainjadiuji(i,2),datatrainjadiuji(i,3),kelas2,kelas2,kelas2,size(kelas2,1));
    x = x+1;
end
...
% ----------------------------------------------------------------
datatrainjadiuji(:,4) = tabelhasil(:,5);
test0 = datatrainjadiuji(find(tabelhasil(:,5)==0),:);
test1 = datatrainjadiuji(find(tabelhasil(:,5)==1),:);
test2 = datatrainjadiuji(find(tabelhasil(:,5)==2),:);

% ---------------------------------------------------------------------
% memindahkan kelas-kelas yang sebenarnya kedalam array 
% tabelhasil pada kolom ke 6 agar dapat dibandingkan ketepatan kelasnya
% ---------------------------------------------------------------------
tabelhasil(:,6) = data_train1(101:150,4);
...
legend('kelas 0 train','kelas 1 train','kelas 2 train','kelas 0 test','kelas 1 test','kelas 2 test');
hold off;

% ------------------------------------
% mencari akurasi dari PNN kodingan ue 
% ------------------------------------
bener = 0;
salah = 0;
for i = 1:size(tabelhasil,1)
    if tabelhasil(i,5) == tabelhasil(i,6)
        bener = bener + 1;
    else 
        salah = salah + 1;
    end
end
akurasi_training = (bener/size(tabelhasil,1))*(100);
